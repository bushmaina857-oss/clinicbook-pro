...
//
// v2: 'claude-3-5-haiku-20241022' has been fully retired on Anthropic's
// side (returns not_found_error, not just a deprecation warning), which
// made every call here fail with a 502 and nothing in the Supabase logs
// (the error was only ever returned to the caller, never console.error'd).
// Fixed both: swapped to the current fast/cheap model, and added logging
// so a future failure shows up in Supabase logs too, not just DevTools.
//
...
//   2. Model name — now 'claude-haiku-4-5-20251001', the current
//      cheap/fast tier, pinned to a dated snapshot rather than the
//      'claude-haiku-4-5' alias so it won't silently shift later.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!claudeRes.ok) {
      ...
      console.error(`summarize-conversation: Anthropic call FAILED (status ${claudeRes.status}):`, errBody);
    ...
    console.error("summarize-conversation: Anthropic call threw:", (e as Error).message);
